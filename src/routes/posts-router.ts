...
postsRouts.get('/',
  async (req: Request, res: Response) => {
    const foundPosts = await ioc.postsService.findPosts(req.query.title?.toString());
    // @ts-ignore
  ...
  .post('/', authMiddlewareHeadersAuthorization,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const newPost = await ioc.postsService.createPost(title, shortDescription, content, bloggerId)
  ...
  .get('/:postId', postIdParamsValidation, inputValidatorMiddleware,
    async (req: Request, res: Response) => {
      try {
        ...
        const getPost = await ioc.postsService.getPostById(postId);
        if (getPost) {
          const postReturn = {
            id: getPost.id,
            title: getPost.title,
  ...
  .put('/:postId', authMiddlewareHeadersAuthorization,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedPost = await ioc.postsService.updatePostById(id, title, shortDescription, content, bloggerId)
  ...
  .delete('/:postId', authMiddlewareHeadersAuthorization,
    ...
    async (req: Request, res: Response) => {
      ...
      const deletedPost = await ioc.postsService.deletedById(id)
